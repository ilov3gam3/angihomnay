    ...
    <div class="container margin_60_40">
        ...
        <div class="text-center mb-4">
            <div class="btn-group" role="group" aria-label="Booking filters">
                <button type="button" class="btn btn-outline-primary filter-btn active" data-filter="ALL">Tất cả
                </button>
                <button type="button" class="btn btn-outline-primary filter-btn" data-filter="BOOKED">Đã đặt (chưa thanh
                    toán)
                </button>
                <button type="button" class="btn btn-outline-primary filter-btn" data-filter="DEPOSITED">Đã thanh toán
                    cọc
                </button>
                <button type="button" class="btn btn-outline-primary filter-btn" data-filter="ONGOING">Đang diễn ra
                </button>
                <button type="button" class="btn btn-outline-secondary filter-btn" data-filter="WAITING_FINAL_PAYMENT">
                    Chờ thanh toán
                </button>
                <button type="button" class="btn btn-outline-danger filter-btn" data-filter="PAID">Đã thanh toán
                </button>
                <button type="button" class="btn btn-outline-success filter-btn" data-filter="CANCELED">Đã hủy</button>
                <button type="button" class="btn btn-outline-success filter-btn" data-filter="NO_SHOW">Không xuất hiện
                </button>
            </div>
        </div>
        <div class="row">
                ...
                <div class="box_booking">
                    <div class="card shadow p-4 rounded-4">
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <a href="<%=request.getContextPath()%>/views/customer/booking-detail.jsp?id=<%=booking.getId()%>">
                                    <h5 class="fw-bold mb-2">Booking ID: <%= booking.getId() %>
                                    </h5>
                                    <h5 class="fw-bold mb-2">Người
                                        đặt: <%= booking.getCustomer().getLastName() + " " + booking.getCustomer().getFirstName() %>
                                    </h5>
                                </a>
                                <p class="mb-1"><strong>Status:</strong> <span
                                        class="badge bg-info"><%= booking.getStatus() %></span></p>
                                <p class="mb-1">
                                    <strong>Table:</strong> <%= booking.getTable() != null ? booking.getTable().getNumber() : "N/A" %>
                                </p>
                                <p class="mb-1"><strong>Number of people:</strong> <%= booking.getPeople() %>
                                </p>
                                <p class="mb-1"><strong>Start:</strong> <%= booking.getStartTime() %>
                                </p>
                                <p class="mb-1"><strong>End:</strong> <%= booking.getEndTime() %>
                                </p>
                                <p class="mb-1"><strong>Note:</strong> <%= booking.getNote() %>
                                </p>
                            </div>
                            <div class="text-end">
                        ...
                        <div class="border rounded p-2 mb-2">
                            <p class="mb-1"><strong>Type:</strong> <%= payment.getType() %>
                            </p>
                            <p class="mb-1"><strong>Amount:</strong> <%= payment.getAmount() %>VND</p>
                            <p class="mb-1"><strong>Bank:</strong> <%= payment.getBankCode() %>
                            </p>
                            ...
                            <p class="mb-0"><strong>Paid At:</strong> <%= payment.getPaid_at() %>
                            </p>
                        </div>
                        <%
                            }
                        ...
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item d-flex justify-content-between">
                                <div><%= detail.getFood().getName() %> x<%= detail.getQuantity() %>
                                </div>
